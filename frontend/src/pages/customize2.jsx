...
function Customize2() {
    ...
    const testVoice = (voiceId) => {
      ...
      if (voiceOption) {
        utterance.text = voiceOption.sample
      }
      ...
      if (voiceId === 'hi-IN-male') {
        utterance.rate = 0.6; // Much slower like UK English
        utterance.pitch = 0.5; // Much lower pitch
      } else {
        utterance.rate = gender === 'male' ? 0.8 : 1.1  // Slower for male, faster for female
        utterance.pitch = gender === 'male' ? 0.7 : 1.3  // Lower pitch for male, higher for female
      }
      ...
      // Try to find voice that matches language and has the right characteristics
      let selectedVoice = null

      // First, try to find voices that explicitly match gender in name
      if (gender === 'male') {
      ...
      // If no gender-specific voice found, use any voice for the language
      if (!selectedVoice) {
      ...
      if (selectedVoice) {
        ...
        console.log(`Selected voice for ${voiceId}:`, selectedVoice.name, "Pitch:", utterance.pitch, "Rate:", utterance.rate)
      }
  ...
  return (
    <div className='w-full min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 flex justify-center items-center flex-col p-[20px] relative overflow-hidden'>
      ...
      <MdKeyboardBackspace className='absolute top-[20px] sm:top-[30px] left-[20px] sm:left-[30px] text-white cursor-pointer w-[20px] h-[20px] sm:w-[25px] sm:h-[25px] hover:scale-110 transition-transform z-20' onClick={()=>navigate("/customize")}/>
      ...
      <div className='w-full max-w-2xl relative z-20'>
        {currentStep === 1 && (
          <div className='text-center'>
            <h1 className='text-white mb-[30px] sm:mb-[40px] text-[24px] sm:text-[30px] bg-gradient-to-r from-purple-400 via-pink-400 to-blue-400 bg-clip-text text-transparent'>Enter Your <span className='text-blue-200'>Assistant Name</span></h1>
            ...
            <div className='mb-8'>
              ...
              />
              {assistantName && (
                <p className='text-white/60 text-sm mt-2'>Your assistant will respond to "{assistantName}"</p>
              )}
            </div>
        ...
        {currentStep === 2 && (
          <div className='text-center'>
            <h1 className='text-white mb-[30px] sm:mb-[40px] text-[24px] sm:text-[30px] bg-gradient-to-r from-purple-400 via-pink-400 to-blue-400 bg-clip-text text-transparent'>Choose Your <span className='text-blue-200'>Assistant Voice</span></h1>
